	...
	public static void main(String[] args) {
		...
		OptionParser parser = new OptionParser() {
			{
				...
				// CloudSpigot Start
				acceptsAll(asList("paper", "paper-settings"), "File for paperspigot settings").withRequiredArg()
		...
		if ((options == null) || (options.has("?"))) {
			...
			try {
				 ...
				 * when using String literals String jline_UnsupportedTerminal = new String(new
				 * char[]
				 * {'j','l','i','n','e','.','U','n','s','u','p','p','o','r','t','e','d','T','e',
				 * 'r','m','i','n','a','l'}); String jline_terminal = new String(new char[]
				 * {'j','l','i','n','e','.','t','e','r','m','i','n','a','l'});
				 * 
				 * useJline =
				 * !(jline_UnsupportedTerminal).equals(System.getProperty(jline_terminal));
				 * 
				 * if (options.has("nojline")) { System.setProperty("user.language", "en");
				 * useJline = false; }
				 * 
				 * if (useJline) { AnsiConsole.systemInstall(); } else { // This ensures the
				 * terminal literal will always match the jline implementation
				...
				// CloudSpigot start
				/*
				 * // Spigot Start int maxPermGen = 0; // In kb for ( String s :
				 * java.lang.management.ManagementFactory.getRuntimeMXBean().getInputArguments()
				 * ) { if ( s.startsWith( "-XX:MaxPermSize" ) ) { maxPermGen = Integer.parseInt(
				 * s.replaceAll( "[^\\d]", "" ) ); maxPermGen <<= 10 * ("kmg".indexOf(
				 * Character.toLowerCase( s.charAt( s.length() - 1 ) ) ) ); } } if (
				 * Float.parseFloat( System.getProperty( "java.class.version" ) ) < 52 &&
				 * maxPermGen < ( 128 << 10 ) ) // 128mb { System.out.println(
