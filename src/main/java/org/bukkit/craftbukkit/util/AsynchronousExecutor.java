...
public final class AsynchronousExecutor<P, T, C, E extends Throwable> {
	...
	public static final AtomicIntegerFieldUpdater STATE_FIELD = AtomicIntegerFieldUpdater
			...
			if (set(this, PENDING, STAGE_1_COMPLETE)) {
				...
				synchronized (this) {
					if (set(this, STAGE_1_SYNC, PENDING)) { // They might NOT synchronized yet, atomic lock IS needed
						 ...
						 * else { // They beat us to the synchronized block }
						 */ // CloudSpigot
				}
			} /*
				 * else { // Async thread is not pending, the more likely situation for a task
				 * not pending }
				 */ // CloudSpigot
		}
