...
public abstract class World implements IBlockAccess {
	...
	// public final List<TileEntity> h = Lists.newArrayList(); // CloudSpigot -
	// Remove unused list
	...
	public ExecutorService lightingExecutor = Executors
			.newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("CloudSpigot - Lighting Thread").build()); // CloudSpigot
																															// -
																															// Asynchronous
																															// lighting
																															// updates
	public final Map<Explosion.CacheKey, Float> explosionDensityCache = new HashMap<Explosion.CacheKey, Float>(); // CloudSpigot
																													// -
																													// Optimize
																													// explosions
	...
	protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider,
		...
		// this.entityLimiter = new
		// org.spigotmc.TickLimiter(spigotConfig.entityMaxTickTime); // CloudSpigot
		// this.tileLimiter = new
		// org.spigotmc.TickLimiter(spigotConfig.tileMaxTickTime); // CloudSpigot
	}
	...
	public void a(int i, int j, int k, int l) {
		...
		if (!this.worldProvider.o()) {
			for (i1 = k; i1 <= l; ++i1) {
				this.updateLight(EnumSkyBlock.SKY, new BlockPosition(i, i1, j)); // CloudSpigot - Asynchronous lighting
																					// updates
			}
		}
	...
	public void applyPhysics(BlockPosition blockposition, Block block) {
		if (captureBlockStates) {
			return;
		} // CloudSpigot - Cancel all physics during placement
	...
	public void a(EntityHuman entityhuman, String s, float f, float f1) {
		for (int i = 0; i < this.u.size(); ++i) {
			((IWorldAccess) this.u.get(i)).a(entityhuman, s, entityhuman.locX, entityhuman.locY, entityhuman.locZ, f,
					f1);
		}
	...
	public List<AxisAlignedBB> getCubes(Entity entity, AxisAlignedBB axisalignedbb) {
		...
		// BlockPosition.MutableBlockPosition blockposition_mutableblockposition = new
		// BlockPosition.MutableBlockPosition(); // CloudSpigot
			...
			for (int chunkz = (i1 >> 4); chunkz <= ((j1 - 1) >> 4); chunkz++) {
				...
				if (chunk == null) {
					...
					if (entity.loadChunks) {
						...
						entity.inUnloadedChunk = true; // CloudSpigot - Remove entities in unloaded chunks
						continue;
					}
					// CloudSpigot end
				}
				...
				for (int x = xstart; x < xend; x++) {
					for (int z = zstart; z < zend; z++) {
						for (int y = ystart; y < l; y++) {
							...
							if (block != null) {
								// CloudSpigot start - FallingBlocks and TNT collide with specific
								// non-collidable blocks
								Block b = block.getBlock();
								if (entity.world.cloudSpigotConfig.fallingBlocksCollideWithSigns
		...
		if (entity instanceof EntityItem)
			return arraylist; // CloudSpigot - Optimize item movement
	...
	public void tickEntities() {
		...
		for (tickPosition = 0; tickPosition < entityList.size(); tickPosition++) {
			...
			if (!entity.dead) {
				try {
					...
					// CloudSpigot start - Prevent tile entity and entity crashes
		...
		if (!this.c.isEmpty()) {
			...
			// this.h.removeAll(this.c); // CloudSpigot - Remove unused list
			this.c.clear();
		}
		// CraftBukkit end
		...
		for (int tileTickPosition = 0; tileTickPosition < tileEntityList.size(); tileTickPosition++) { // CloudSpigot -
																										// Disable tick
																										// limiters
	...
	public boolean a(TileEntity tileentity) {
		boolean flag = true; // CloudSpigot - Remove unused list
	...
	public void a(Collection<TileEntity> collection) {
		if (this.M) {
			...
			while (iterator.hasNext()) {
				...
				// this.h.add(tileentity); // CloudSpigot - Remove unused list
	...
	public void entityJoinedWorld(Entity entity, boolean flag) {
		...
		if (!org.spigotmc.ActivationRange.checkIfActive(entity)) {
			...
			if (!this.isChunkLoaded(i, j, true) && ((entity instanceof EntityEnderPearl
					&& this.cloudSpigotConfig.removeUnloadedEnderPearls)
			...
			if (!entity.ad || entity.ae != k || entity.af != l || entity.ag != i1) {
				if (entity.loadChunks)
					entity.loadChunks(); // CloudSpigot - Force load chunks
	...
	public boolean a(AxisAlignedBB axisalignedbb, Entity entity) {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			// CloudSpigot start - Allow block placement if the placer cannot see the
			// vanished blocker
			if (entity instanceof EntityPlayer && entity1 instanceof EntityPlayer && (!((EntityPlayer) entity)
					.getBukkitEntity().canSee(((EntityPlayer) entity1).getBukkitEntity()))) {
				continue;
			}
	...
	protected void a(int i, int j, Chunk chunk) {
		...
		if (!this.cloudSpigotConfig.disableMoodSounds && this.L == 0 && !this.isClientSide) { // CloudSpigot - Disable
																								// mood sounds
	...
	public boolean x(BlockPosition blockposition) {
		...
		if (!this.worldProvider.o()) {
			flag |= this.updateLight(EnumSkyBlock.SKY, blockposition); // CloudSpigot - Asynchronous lighting updates
		}

		flag |= this.updateLight(EnumSkyBlock.BLOCK, blockposition); // CloudSpigot - Asynchronous lighting updates
		return flag;
	}
	...
	public boolean c(EnumSkyBlock enumskyblock, BlockPosition blockposition, Chunk chunk, List<Chunk> neighbors) { // CloudSpigot
	...
	/**
	 * CloudSpigot - Asynchronous lighting updates
	 */
	...
	// CloudSpigot start - Modified methods for affects spawning
