...
public class ChunkProviderServer implements IChunkProvider {
	...
	public void queueUnload(int i, int j) {
		...
		if (chunk != null && chunk.world.cloudSpigotConfig.useAsyncLighting
				&& (chunk.pendingLightUpdates.get() > 0 || chunk.world.getTime() - chunk.lightUpdateTime < 20)) {
			return;
		}
		// CloudSpigot end
		// CloudSpigot start - Don't unload chunk if it contains an entity that loads
		// chunks
		if (chunk != null) {
	...
	public Chunk getChunkAt(int i, int j, Runnable runnable) {
		...
		if (chunk == null && loader != null && loader.chunkExists(world, i, j)) {
			if (runnable != null) {
				...
				// CloudSpigot start - If there was an issue loading the chunk from region,
				// stage1 will fail and stage2 will load it sync
			...
			if (chunk == null) {
				if (this.chunkProvider == null) {
					...
					try {
						...
						crashreportsystemdetails.a("Position hash", Long.valueOf(LongHash.toLong(i, j))); // CraftBukkit
																											// -
																											// Use
																											// LongHash
	...
	public boolean saveChunks(boolean flag, IProgressUpdate iprogressupdate) {
		...
		while (iterator.hasNext()) {
			...
			if (chunk.a(flag)) {
				...
				chunk.f(false);
				/*
				 * ++i; if (i == 24 && !flag && false) { // Spigot return false; }
				 */ // CloudSpigot
			}
		}
