...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
	...
	public DedicatedServer(joptsimple.OptionSet options) {
		...
		// CraftBukkit end
		/*
		 * Thread thread = new Thread("Server Infinisleeper") { { this.setDaemon(true);
		 * this.start(); }
		 * 
		 * public void run() { while (true) { try { Thread.sleep(2147483647L); } catch
	...
	protected boolean init() throws IOException {
		Thread thread = new Thread("Server console handler") {
			public void run() {
				 ...
				 * 
				 * try { // CraftBukkit start - JLine disabling compatibility while
				 * (!isStopped() && isRunning()) { if (org.bukkit.craftbukkit.Main.useJline) { s
				 * = bufferedreader.readLine(">", null); } else { s = bufferedreader.readLine();
				 * } if (s != null && s.trim().length() > 0) { // Trim to filter lines which are
				 * just spaces issueCommand(s, DedicatedServer.this); } // CraftBukkit end } }
		 ...
		 * ((org.apache.logging.log4j.core.Logger) LogManager.getRootLogger()); for
		 * (org.apache.logging.log4j.core.Appender appender :
		 * logger.getAppenders().values()) { if (appender instanceof
		...
		thread.start();
		DedicatedServer.LOGGER.info("Starting minecraft server version 1.8.10");
		...
		if (!p.a()) { // Spigot
			...
			if (!NameReferencingFileConverter.a(this.propertyManager)) {
				...
				/*
				 * if (false && this.aS() > 0L) { // Spigot - disable Thread thread1 = new
				 * Thread(new ThreadWatchdog(this));
				 * 
				 * thread1.setName("Server Watchdog"); thread1.setDaemon(true); thread1.start();
				 * }
				 */ // CloudSpigot
	...
	public void a(MojangStatisticsGenerator mojangstatisticsgenerator) {
		...
		/*
		 * mojangstatisticsgenerator.a("whitelist_enabled",
		 * Boolean.valueOf(this.aP().getHasWhitelist()));
		 * mojangstatisticsgenerator.a("whitelist_count",
		 * Integer.valueOf(this.aP().getWhitelisted().length));
