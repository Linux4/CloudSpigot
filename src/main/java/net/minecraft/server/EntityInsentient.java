...
public abstract class EntityInsentient extends EntityLiving {
	...
	public PathfinderGoalFloat goalFloat; // CloudSpigot
	...
	protected void D() {
		if (this.persistent) {
			...
			EntityHuman entityhuman = this.world.findNearbyPlayerWhoAffectsSpawning(this, -1.0D); // CloudSpigot -
																									// Affects Spawning
																									// API
			...
			if (entityhuman != null) {
				...
				if (d3 > this.world.cloudSpigotConfig.hardDespawnDistance) { // CraftBukkit - remove
																				// isTypeNotPersistent() check //
																				// CloudSpigot - custom despawn
																				// distances
					this.die();
				}

				if (this.ticksFarFromPlayer > 600 && this.random.nextInt(800) == 0
						&& d3 > this.world.cloudSpigotConfig.softDespawnDistance) { // CraftBukkit - remove
																					// isTypeNotPersistent() check //
																					// CloudSpigot - custom despawn
																					// distance
					this.die();
				} else if (d3 < this.world.cloudSpigotConfig.softDespawnDistance) { // CloudSpigot - custom despawn
																					// distances
					this.ticksFarFromPlayer = 0;
				}
			}
	...
	protected final void doTick() {
		...
		if (this.fromMobSpawner) {
			// CloudSpigot start - Allow nerfed mobs to jump
			if (goalFloat != null) {
				if (goalFloat.a())
					goalFloat.e();
				this.g.b();
	...
	private void n() {
		if (this.bo && this.bq != null) {
			if (this.bq.hasKeyOfType("UUIDMost", 4) && this.bq.hasKeyOfType("UUIDLeast", 4)) {
				...
				List<EntityLiving> list = this.world.a(EntityLiving.class,
						this.getBoundingBox().grow(10.0D, 10.0D, 10.0D));
