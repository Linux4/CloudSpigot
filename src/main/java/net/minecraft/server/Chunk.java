...
import java.util.concurrent.atomic.AtomicInteger; // CloudSpigot
...
public class Chunk {
	...
	private boolean k;
	public volatile Map<BlockPosition, TileEntity> tileEntities; // CloudSpigot
	public volatile List<Entity>[] entitySlices; // Spigot // CloudSpigot
	// CloudSpigot start - track the number of minecarts and items
	...
	// CloudSpigot start - ChunkMap caching
	...
	public PacketPlayOutMapChunk.ChunkMap getChunkMap(boolean groundUpContinuous, int primaryBitMask) {
		if (!world.cloudSpigotConfig.cacheChunkMaps || !groundUpContinuous
	...
	private void a(int i, int j, int k, int l) {
		if (l > k && this.world.areChunksLoaded(new BlockPosition(i, 0, j), 16)) {
			for (int i1 = k; i1 < l; ++i1) {
				this.world.updateLight(EnumSkyBlock.SKY, new BlockPosition(i, i1, j)); // CloudSpigot - Asynchronous
																						// lighting updates
			}
	...
	// CloudSpigot start - Optimize getBlockData
	...
	public void a(Entity entity) {
		...
		// CloudSpigot start - update counts
	...
	public void a(BlockPosition blockposition, TileEntity tileentity) {
		...
		if (this.getType(blockposition) instanceof IContainer) {
			...
			ServerInternalException e = new ServerInternalException("Attempted to place a tile entity (" + tileentity
					...
					+ tileentity.position.getZ() + " (" + CraftMagicNumbers.getMaterial(getType(blockposition))
					+ ") where there was no entity tile!\n" + "Chunk coordinates: " + (this.locX * 16) + ","
					+ (this.locZ * 16));
			e.printStackTrace();
	...
	public void a(Entity entity, AxisAlignedBB axisalignedbb, List<Entity> list, Predicate<? super Entity> predicate) {
		...
		for (int k = i; k <= j; ++k) {
			if (!this.entitySlices[k].isEmpty()) {
				...
				// CloudSpigot start - Don't search for inventories if we have none, and that is
				// all we want
				/*
	...
	public <T extends Entity> void a(Class<? extends T> oclass, AxisAlignedBB axisalignedbb, List<T> list,
		...
		for (int k = i; k <= j; ++k) {
			if (counts != null && counts[k] <= 0)
				continue; // CloudSpigot - Don't check a chunk if it doesn't have the type we are looking
							// for
