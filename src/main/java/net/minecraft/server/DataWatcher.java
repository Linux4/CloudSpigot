...
public class DataWatcher {
	...
	private static final gnu.trove.map.TObjectIntMap<Class> classToId = new gnu.trove.map.hash.TObjectIntHashMap<Class>(
			10, 0.5f, -1);
	private final gnu.trove.map.TIntObjectMap<WatchableObject> dataValues = new gnu.trove.map.hash.TIntObjectHashMap<WatchableObject>(
			10, 0.5f, -1);
	// These exist as an attempt at backwards compatability for (broken) NMS plugins
	// private static final Map<Class<?>, Integer> c = gnu.trove.TDecorators.wrap(
	// classToId ); // CloudSpigot
	// private final Map<Integer, DataWatcher.WatchableObject> d =
	// gnu.trove.TDecorators.wrap( dataValues ); // CloudSpigot
	// Spigot End
	private boolean e;
