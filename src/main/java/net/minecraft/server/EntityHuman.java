...
public abstract class EntityHuman extends EntityLiving {
	...
	private Collection e(Entity entity) { // CraftBukkit - TODO: Check me?
		...
		if (scoreboardteam != null) {
			...
			if (i >= 0 && i < IScoreboardCriteria.i.length) {
				Iterator<ScoreboardObjective> iterator = this.getScoreboard()
						.getObjectivesForCriteria(IScoreboardCriteria.i[i]).iterator();
	...
	public boolean damageEntity(DamageSource damagesource, float f) {
		if (this.isInvulnerable(damagesource)) {
			...
			if (this.getHealth() <= 0.0F) {
				...
				/*
				 * if (false && f == 0.0F) { // CraftBukkit - Don't filter out 0 damage return
				 * false; } else {
				 */ // CloudSpigot
	...
	protected boolean d(DamageSource damagesource, float f) { // void -> boolean
		...
		// CraftBukkit end
		/*
		 * if (!this.isInvulnerable(damagesource)) { if (!damagesource.ignoresArmor() &&
		 * this.isBlocking() && f > 0.0F) { f = (1.0F + f) *
		 * this.world.cloudSpigotConfig.playerBlockingDamageMultiplier; // CloudSpigot -
		 * Configurable damage multiplier for blocking; }
		 * 
		 * f = this.applyArmorModifier(damagesource, f); f =
		 * this.applyMagicModifier(damagesource, f); float f1 = f;
		 * 
		 * f = Math.max(f - this.getAbsorptionHearts(), 0.0F);
		 * this.setAbsorptionHearts(this.getAbsorptionHearts() - (f1 - f)); if (f !=
		 * 0.0F) { this.applyExhaustion(damagesource.getExhaustionCost()); float f2 =
		 * this.getHealth();
		 * 
		 * this.setHealth(this.getHealth() - f); this.bs().a(damagesource, f2, f); if (f
		 * < 3.4028235E37F) { this.a(StatisticList.x, Math.round(f * 10.0F)); }
		 * 
	...
	public void attack(Entity entity) {
		if (entity.aD()) {
			if (!entity.l(this)) {
				...
				if (f > 0.0F || f1 > 0.0F) {
					boolean flag = !world.cloudSpigotConfig.disablePlayerCrits && this.fallDistance > 0.0F
							&& !this.onGround && !this.k_() && !this.V() && !this.hasEffect(MobEffectList.BLINDNESS)
							&& this.vehicle == null && entity instanceof EntityLiving; // CloudSpigot
					...
					if (flag2) {
						if (i > 0) {
							entity.g((double) (-MathHelper.sin(this.yaw * 3.1415927F / 180.0F) * (float) i * 0.5F),
									0.1D, (double) (MathHelper.cos(this.yaw * 3.1415927F / 180.0F) * (float) i * 0.5F));
							this.motX *= 0.6D;
							this.motZ *= 0.6D;
	...
	// CloudSpigot start - SPIGOT-1387: Resolve bed issues on unloaded chunks
		...
		if (this.vehicle == null) {
			...
			if (this.a(Material.WATER)) {
				...
				if (i > 0) {
					...
					this.applyExhaustion(world.cloudSpigotConfig.playerSwimmingExhaustion * (float) i * 0.01F); // CloudSpigot
																												// -
																												// Configurable
																												// swimming
																												// exhaustion
