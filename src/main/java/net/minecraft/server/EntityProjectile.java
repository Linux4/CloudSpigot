...
public abstract class EntityProjectile extends Entity implements IProjectile {
	...
	public void t_() {
		...
		// CloudSpigot start - Allow projectiles to fly through vanished players the
		// shooter can't see
		...
		for (this.pitch = (float) (MathHelper.b(this.motY, (double) f1) * 180.0D / 3.1415927410125732D); this.pitch
		...
		if (this.V()) {
			for (int j = 0; j < 4; ++j) {
				...
				this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX - this.motX * (double) f4,
						this.locY - this.motY * (double) f4, this.locZ - this.motZ * (double) f4, this.motX, this.motY,
						this.motZ, new int[0]);
			}
