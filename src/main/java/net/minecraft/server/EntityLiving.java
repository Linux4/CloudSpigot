...
public abstract class EntityLiving extends Entity {
		...
		if (i > 0) {
			...
			if (flag1 && i > 0) {
				...
				this.world.addParticle(flag ? EnumParticle.SPELL_MOB_AMBIENT : EnumParticle.SPELL_MOB,
						...
						this.locZ + (this.random.nextDouble() - 0.5D) * this.width, d0, d1, d2, new int[0]);
			}
		}
	...
	public boolean damageEntity(DamageSource damagesource, float f) {
		if (this.isInvulnerable(damagesource)) {
			...
			if (this.getHealth() <= 0.0F) {
				...
				if (flag && !(knockbackCancelled = world.cloudSpigotConfig.disableExplosionKnockback
				...
				if (knockbackCancelled)
					this.world.broadcastEntityEffect(this, (byte) 2); // CloudSpigot
	...
	protected float applyMagicModifier(DamageSource damagesource, float f) {
		if (damagesource.isStarvation()) {
			 ...
			 * DamageSource.OUT_OF_WORLD) { i =
			 * (this.getEffect(MobEffectList.RESISTANCE).getAmplifier() + 1) * 5; j = 25 -
			 * i; f1 = f * (float) j; f = f1 / 25.0F; }
			 */ // CloudSpigot
	...
	public void mount(Entity entity) {
		if (this.vehicle != null && entity == null) {
			...
			// CraftBukkit end
			// CloudSpigot start - make dismountEvent cancellable
