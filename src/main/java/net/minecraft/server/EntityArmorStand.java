...
public class EntityArmorStand extends EntityLiving {
	...
	public boolean a(EntityHuman entityhuman, Vec3D vec3d) {
		if (this.s()) {
			...
			/*
			 * double d0 = 0.1D; double d1 = 0.9D; double d2 = 0.4D; double d3 = 1.6D;
			 */ // CloudSpigot
			byte b1 = 0;
	...
	private void a(EntityHuman entityhuman, int i) {
		...
		if ((itemstack == null || (this.bi & 1 << i + 8) == 0) && (itemstack != null || (this.bi & 1 << i + 16) == 0)) {
			...
			if (entityhuman.abilities.canInstantlyBuild
					&& (itemstack == null || itemstack.getItem() == Item.getItemOf(Blocks.AIR)) && itemstack1 != null) {
	...
	public boolean damageEntity(DamageSource damagesource, float f) {
		...
		if (this.world.isClientSide || this.dead) {
			return false;
		} else if (DamageSource.OUT_OF_WORLD.equals(damagesource)) { // CloudSpigot
			this.die();
			return false;
