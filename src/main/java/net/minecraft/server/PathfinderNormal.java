            ...
            for (int l1 = j; l1 < j + i1; ++l1) {
                for (int i2 = k; i2 < k + j1; ++i2) {
                    ...
                    if (block.getMaterial() != Material.AIR) {
                        ...
                        if (block instanceof BlockMinecartTrackAbstract) { // CloudSpigot - Pathfinder optimizations
                            if (!(iblockaccess.getType(blockposition).getBlock() instanceof BlockMinecartTrackAbstract) && !(iblockaccess.getType(blockposition.down()).getBlock() instanceof BlockMinecartTrackAbstract)) { // CloudSpigot - Pathfinder optimizations
                                return -3;
                            }
