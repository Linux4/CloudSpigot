...
public class WorldServer extends World implements IAsyncTaskHandler {
	...
	private boolean O;
	// private int emptyTime; // CloudSpigot
	...
	public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
		...
		this.Q = ((org.bukkit.craftbukkit.CraftTravelAgent) new org.bukkit.craftbukkit.CraftTravelAgent(this)
				.setSearchRadius(cloudSpigotConfig.portalSearchRadius)); // CraftBukkit // Paper - configurable search
																			// radius
		this.B();
		this.C();
	...
	public BiomeBase.BiomeMeta a(EnumCreatureType enumcreaturetype, BlockPosition blockposition) {
		List<BiomeMeta> list = this.N().getMobsFor(enumcreaturetype, blockposition);
	...
	protected void h() {
		...
		if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
			...
			for (gnu.trove.iterator.TLongShortIterator iter = chunkTickList.iterator(); iter.hasNext();) {
				...
				if (!this.cloudSpigotConfig.disableThunder && this.random.nextInt(100000) == 0 && this.S()
						&& this.R()) { // CloudSpigot - Disable thunder
				...
				if (!this.cloudSpigotConfig.disableIceAndSnow && this.random.nextInt(16) == 0) { // CloudSpigot -
																									// Disable ice and
																									// snow
					this.m = this.m * 3 + 1013904223;
					i1 = this.m >> 2;
			...
			if (block.N()) {
				...
				if (this.areChunksLoadedBetween(nextticklistentry.a.a(-b0, -b0, -b0),
					...
					if (iblockdata.getBlock().getMaterial() != Material.AIR
						...
						stopPhysicsEvent = !cloudSpigotConfig.firePhysicsEventForRedstone
								&& (iblockdata.getBlock() instanceof BlockDiodeAbstract
										|| iblockdata.getBlock() instanceof BlockRedstoneTorch); // CloudSpigot
	...
	public void tickEntities() {
		/*
		 * if (false && this.players.isEmpty()) { // CraftBukkit - this prevents entity
		 * cleanup, other issues on servers with no players if (this.emptyTime++ >=
		 * 1200) { return; } } else {
		 */ // CloudSpigot
		this.j();
		// } // CloudSpigot
	...
	public boolean a(boolean flag) {
		if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
			...
			/*
			 * if (false) { // CraftBukkit throw new
			 * IllegalStateException("TickNextTick list out of sync"); } else {
			 */ // CloudSpigot
	...
	/*
	 * private boolean getSpawnNPCs() { return this.server.getSpawnNPCs(); }
	 */ // CloudSpigot

	/*
	 * private boolean getSpawnAnimals() { return this.server.getSpawnAnimals(); }
	 */ // CloudSpigot
	...
	public void save(boolean flag, IProgressUpdate iprogressupdate) throws ExceptionWorldConflict {
		if (this.chunkProvider.canSave()) {
			...
			Iterator<Chunk> iterator = arraylist.iterator();
	...
	private void ak() {
		while (!this.S[this.T].isEmpty()) {
			...
			this.T ^= 1;
			Iterator<BlockActionData> iterator = this.S[i].iterator();

			while (iterator.hasNext()) {
				BlockActionData blockactiondata = iterator.next();
